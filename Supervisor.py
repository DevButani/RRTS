...
"""
new_complaints_file="new_comp.csv"
complaints_file="comp.csv"
locality='A'

new_complaints_df=pd.read_csv(new_complaints_file)
# complaints_df=pd.read_csv(complaints_file)

filterd_new_complaints_df=new_complaints_df[new_complaints_df['Locality']==locality]
filterd_new_complaints_df.reset_index(inplace=True, drop=True)

complaints_df=pd.read_csv(complaints_file)

new_complaints_df.drop(new_complaints_df[new_complaints_df['Locality']==locality].index, inplace=True)
new_complaints_df.reset_index(drop=True, inplace=True)
new_complaints_df.to_csv(new_complaints_file, index=False)

display_df = complaints_df[(complaints_df['Status']=='In Progress') & (complaints_df['Locality']==locality)]
complaints_df.drop(complaints_df[(complaints_df['Status']=='In Progress') & (complaints_df['Locality']==locality)].index, inplace=True)
...
def show_form(complaint_no):
    global current_complaint_no
    current_complaint_no=complaint_no
    show_frame(complaint_form_frame)
    street_label.config(text=filterd_new_complaints_df['Street'][complaint_no])
    problem_label.config(text=filterd_new_complaints_df['Problem'][complaint_no])
    reporting_date_label.config(text=filterd_new_complaints_df['Reporting Date'][complaint_no])


#new complaints
def show_new_complaints():
    # show_frame(loading_frame)
    global filterd_new_complaints_df
    ...
    no_of_complaints=len(filterd_new_complaints_df.index)
    if(no_of_complaints>0):
        ...
        for i in range(no_of_complaints):
            problem = Label(new_complaints_canvas, text= filterd_new_complaints_df['Problem'][i]+" at "+filterd_new_complaints_df['Street'][i], bg="yellow", fg="red")
...
def submit():
    global filterd_new_complaints_df, current_complaint_no
    temp_dict=[{'Locality':locality, 'Street':filterd_new_complaints_df['Street'][current_complaint_no], 'Problem':filterd_new_complaints_df['Problem'][current_complaint_no], 'Reporting Date':filterd_new_complaints_df['Reporting Date'][current_complaint_no], 'Severity': severity_selection.get(), 'Traffic': traffic_selection.get(), 'Asphalt': asphalt_quantity.get(), 'Bitumen': bitumen_quantity.get(), 'Concrete': concrete_quantity.get(), 'Road Roller': roller_quantity.get(), 'Bulldozer': bulldozer_quantity.get(), 'Concrete Mixer': mixer_quantity.get(), 'Jackhammer': jackhammer_quantity.get(), 'Engineer': engineer_quantity.get(), 'Worker': worker_quantity.get(), 'Operator': operator_quantity.get(), 'Status': 'Pending', 'Completion Date': ""}]
    temp_df=pd.DataFrame(temp_dict)
    temp_df.to_csv(complaints_file, mode='a', header=False, index=False)
    filterd_new_complaints_df.drop(current_complaint_no, inplace=True)
    filterd_new_complaints_df.reset_index(drop=True, inplace=True)
...
display_df.to_csv(complaints_file, mode='a', header=False, index=False)
filterd_new_complaints_df.to_csv(new_complaints_file, mode='a', header=False, index=False)